...
def _probe_rs_tools_toolchain_impl(ctx):
    ...
    template_variables = platform_common.TemplateVariableInfo({
        "PROBE_RS_TOOLS_BIN": target_tool_path,
    })
    default = DefaultInfo(
        files = depset(tool_files),
        runfiles = ctx.runfiles(files = tool_files),
    )
    probe_rs_toolsinfo = ProbeRsToolsInfo(
    ...
    toolchain_info = platform_common.ToolchainInfo(
        probe_rs_toolsinfo = probe_rs_toolsinfo,
...
probe_rs_tools_toolchain = rule(
    ...
    doc = """Defines a probe_rs_tools compiler/runtime toolchain.
